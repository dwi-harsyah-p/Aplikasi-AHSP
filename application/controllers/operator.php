...
class Operator extends CI_Controller
{
    ...
    public function bahan()
    {
        ...
        $data['user_role'] = $this->Ahsp_model->getTablewhere('user', 'nip', $this->session->userdata('nip'))->row_array();
        $data['role'] = $this->Ahsp_model->getTablewhere('user_role', 'id', $data['user_role']['role_id'])->row_array();
        $data['daerah'] = $this->db->query("SELECT daerah FROM daerah INNER JOIN biodata ON biodata.id_daerah = daerah.id WHERE biodata.nip = " . $data['user']['nip'])->row_array();        
        if ($data['role']['role'] == 'User') {            
            $this->load->view('operator/bahan/indexuser', $data);
        }else {            
    ...
    public function upah()
    {
        ...
        $data['daerah'] = $this->db->query("SELECT daerah FROM daerah INNER JOIN biodata ON biodata.id_daerah = daerah.id WHERE biodata.nip = " . $data['user']['nip'])->row_array();        
        if ($data['role']['role'] == 'User') {            
            $this->load->view('operator/upah/indexuser', $data);
        }else { 
    ...
    public function alat()
    {
        ...
        $data['daerah'] = $this->db->query("SELECT daerah FROM daerah INNER JOIN biodata ON biodata.id_daerah = daerah.id WHERE biodata.nip = " . $data['user']['nip'])->row_array();        
        if ($data['role']['role'] == 'User') {            
            $this->load->view('operator/alat/indexuser', $data);
        }else { 
